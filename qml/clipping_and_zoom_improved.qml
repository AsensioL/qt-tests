import QtQuick
import QtQuick.Controls
import QtMultimedia

ApplicationWindow {
    id:       mainWindow
    visible:  true
    title:    "Animation App"
    width:  1280 + 100
    height:  720 + 100

    // Main QML content goes here
    Rectangle {
        width:  1280
        height:  720

        color: "darkgrey"

        Rectangle {
            id: container
            x: 20
            y: 20
            width:  800*1.5
            height: 400*1.5
            color: "yellow"
            clip: true

            Rectangle {
                id:     noGoZone
                x:      imagenCentro.zoomOutLeftMargin
                y:      imagenCentro.zoomOutTopMargin
                width:  imagenCentro.width  - (imagenCentro.zoomOutLeftMargin + imagenCentro.zoomOutRightMargin)
                height: imagenCentro.height - (imagenCentro.zoomOutTopMargin + imagenCentro.zoomOutBottomMargin)
                color:  "red"
            }

            Image {
                id: imagenCentro
                x: 0
                y: 0
                width:  800*1.5
                height: 400*1.5
                fillMode: Image.PreserveAspectCrop
                source: "data:image/jpeg;base64,"

                transformOrigin: Item.TopLeft
                scale: 1

                property int zoomOutLeftMargin:   0; // px
                property int zoomOutRightMargin:  0; // px
                property int zoomOutTopMargin:    0; // px
                property int zoomOutBottomMargin: 0; // px

                property bool zoomOutAllowed: true
                property bool zoomInAllowed:  true

                property vector2d coord: Qt.vector2d(x, y)
                function setCoord(point) {
                    x = point.x
                    y = point.y
                }

                // Calculate the position of a pixel in the unscaled image corresponding to a
                // ViewPort pixel at the current settings of scale and offset
                function viewPortToImageCoord(viewPortCoord) {
                    var deltaX = viewPortCoord.x - coord.x;
                    var deltaY = viewPortCoord.y - coord.y;
                    return Qt.vector2d(deltaX / scale, deltaY / scale);
                }

                // Given a position in the unscaled image (imageCoord) and where it should be
                // in the ViewPort (viewPortCoord) at a given zoom level (newScale), calculate the
                // required offset.
                function calculateOffsetCoord(viewPortCoord, imageCoord, newScale) {
                    var x = viewPortCoord.x - imageCoord.x * newScale;
                    var y = viewPortCoord.y - imageCoord.y * newScale;
                    return Qt.vector2d(x, y);
                }

                // Given a position in the ViewPort (viewPortCoord), the current View Port offset
                // (viewPortOffsetCoord) and the ratio of scaling (scaleRatio = newScale / scale),
                // calculate the required offset to keep viewPortCoord in the same position.
                // Optimizes: calculateOffsetCoord
                function calculateOffsetCoord2(viewPortCoord, viewPortOffsetCoord, scaleRatio) {
                    return Qt.vector2d(viewPortCoord.x * (1 - scaleRatio) + viewPortOffsetCoord.x * scaleRatio,
                                    viewPortCoord.y * (1 - scaleRatio) + viewPortOffsetCoord.y * scaleRatio);
                }

                // Prevent infinite zoom out by limitting the scale
                function protectZoomOut(newScale) {
                    // Limit zoom out
                    var originalScale = newScale
                    if (width * newScale < width - (zoomOutLeftMargin + zoomOutRightMargin)) {
                        newScale = 1 - (zoomOutLeftMargin + zoomOutRightMargin) / width
                        zoomOutAllowed = false
                    }
                    else if (height * newScale < height - (zoomOutTopMargin + zoomOutBottomMargin)) {
                        newScale = 1 - (zoomOutTopMargin + zoomOutBottomMargin) / height
                        zoomOutAllowed = false
                    }
                    else {
                        zoomOutAllowed = true
                    }
                    return newScale
                }

                // Prevent shifting the out of bounds by limiting the X/Y offset
                // This allows some "black space" with configurable sizes on each of the top-left-right-bottom
                function protectOffset(offsetCoord, newScale) {
                    var scaledWidth  = width  * newScale
                    var scaledHeight = height * newScale
                    var scaledRight  = offsetCoord.x + width  * newScale
                    var scaledBottom = offsetCoord.y + height * newScale
                    var check_bitfield = 0

                    // Left side can't start more to the right than the allowed left side-margin (zoomOutLeftMargin)
                    if (offsetCoord.x > zoomOutLeftMargin) {
                        offsetCoord.x = zoomOutLeftMargin
                        check_bitfield |= 1
                    }
                    // Top side can't star lower than the allowed top-side margin (zoomOutTopMargin)
                    if (offsetCoord.y > zoomOutTopMargin) {
                        offsetCoord.y = zoomOutTopMargin
                        check_bitfield |= 2
                    }
                    // Ride side can't end more to the left than the allowed right side-margin (zoomOutRightMargin)
                    if (scaledRight < width - zoomOutRightMargin) {
                        offsetCoord.x = width - zoomOutRightMargin - width  * newScale
                        check_bitfield |= 4
                    }
                    // Bottom side can't end higher than the allowed bottom-side margin (zoomOutTopMargin)
                    if (scaledBottom < height - zoomOutBottomMargin) {
                        offsetCoord.y = height - zoomOutBottomMargin - height  * newScale
                        check_bitfield |= 8
                    }
                    console.assert((check_bitfield & (1 | 4)) != (1 | 4), "2 horizontal constraints applied")
                    console.assert((check_bitfield & (2 | 8)) != (2 | 8), "2 vertical constraints applied  ")
                }

                function handleZoomUpdate(viewPortCoord, scaleRatio, viewPortCoordDelta) {
                    // Don't apply zoom if it is not allowed
                    if ( (!zoomOutAllowed && scaleRatio < 0) ||
                         ( !zoomInAllowed && scaleRatio > 0)  ) {
                        return
                    }

                    var viewPortOffsetCoord = coord.plus(viewPortCoordDelta)
                    //var imageCoord     = viewPortToImageCoord(viewPortCoord)
                    //var newOffsetCoord = calculateOffsetCoord(viewPortCoord, imageCoord, newScale)
                    var newOffsetCoord = calculateOffsetCoord2(viewPortCoord, viewPortOffsetCoord, scaleRatio)
                    newOffsetCoord = newOffsetCoord

                    // Prevent out-of-frame offsets/scales
                    var newScale = protectZoomOut(scale * scaleRatio)
                    protectOffset(newOffsetCoord, newScale)

                    // Apply zoom
                    imagenCentro.scale = newScale
                    setCoord(newOffsetCoord)
                }

                function reset() {
                    imagenCentro.scale = 1
                    setCoord(Qt.vector2d(0,0))
                }
            }


            MouseArea {
                anchors.fill: parent
                acceptedButtons: Qt.AllButtons

                property real increments: 1.1

                onWheel: (wheel) => {
                    var viewPortCoord      = Qt.vector2d(wheel.x, wheel.y)
                    var scaleRatio         = Math.pow(increments, Math.round(wheel.angleDelta.y / 120))
                    var viewPortCoordDelta = Qt.vector2d(0, 0) // The wheel does not move the offset except for by zooming in/out

                    imagenCentro.handleZoomUpdate(viewPortCoord, scaleRatio, viewPortCoordDelta)
                }

                onClicked: (mouse) => {
                    if (mouse.button == Qt.LeftButton) {
                        var viewPortCoord = Qt.vector2d(mouse.x, mouse.y)
                        console.log(`ViewPort position: ${viewPortCoord}`)
                    }
                    else if (mouse.button == Qt.RightButton) {
                        var viewPortCoord = Qt.vector2d(mouse.x, mouse.y)
                        var imageCoord = imagenCentro.viewPortToImageCoord(viewPortCoord)
                        console.log(`Image position:    ${imageCoord}`)
                    }
                    else if (mouse.button == Qt.MiddleButton) {
                        imagenCentro.reset()
                    }
                }
            }

            PinchArea {
                anchors.fill: parent

                onPinchUpdated: (pinch) => {
                    var viewPortCoord     = Qt.vector2d(pinch.center.x, pinch.center.y)
                    var viewPortPrevCoord = Qt.vector2d(pinch.previousCenter.x, pinch.previousCenter.y)
                    var viewPortCoordDelta = viewPortCoord.minus(viewPortPrevCoord)
                    var scaleRatio = pinch.scale / pinch.previousScale

                    imagenCentro.handleZoomUpdate(viewPortCoord, scaleRatio, viewPortCoordDelta)
                }
            }
        }
    }
}