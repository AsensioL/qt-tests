    ...
    Rectangle {
        ...
        Rectangle {
            ...
            Image {
                ...
                // Given the offset coordinates and the scale, prevent infinite zoom out by
                // setting (zoomOutAllowed). This also recenters the image as zooming out occurs
                // while allowing "black space" with configurable sizes on each of the top-left-right-bottom
                function protectZoomOut(scale, offsetCoord) {
                    // Limit zoom out
                    var originalScale = scale
                    if (width * scale < width - (zoomOutLeftMargin + zoomOutRightMargin)) {
                        scale = 1 - (zoomOutLeftMargin + zoomOutRightMargin) / width
                        zoomOutAllowed = false
                    }
                    else if (height * scale < height - (zoomOutTopMargin + zoomOutBottomMargin)) {
                        scale = 1 - (zoomOutTopMargin + zoomOutBottomMargin) / height
                        zoomOutAllowed = false
                    }
                    else {
                        zoomOutAllowed = true
                    ...
                    var scaledRight  = offsetCoord.x + width  * scale
                    var scaledBottom = offsetCoord.y + height * scale
                    ...
                    if (scaledRight < width - zoomOutRightMargin) {
                        offsetCoord.x = width - zoomOutRightMargin - width  * scale
                        check_bitfield |= 4
                    }
                    ...
                    if (scaledBottom < height - zoomOutBottomMargin) {
                        offsetCoord.y = height - zoomOutBottomMargin - height  * scale
                        check_bitfield |= 8
                    }
                ...
                function applyZoom(viewPortCoord, wheelAngleDelta) {
                    ...
                    if ( (!zoomOutAllowed && wheelAngleDelta < 0) ||
                         ( !zoomInAllowed && wheelAngleDelta > 0)  ) {
                        return
                    }

                    steps += wheelAngleDelta
                    //console.log(`viewPortCoord ${viewPortCoord}, wheelAngleDelta ${wheelAngleDelta}, steps ${steps}`)

                    var newScale       = Math.pow(increments, Math.round(steps / 120))
                    ...
                    newScale = protectZoomOut(newScale, newOffsetCoord)
